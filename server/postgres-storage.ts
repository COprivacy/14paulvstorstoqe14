...
export class PostgresStorage implements IStorage {
  ...
  async getCupons(): Promise<Cupom[]> {
    try {
      const result = await this.db
        .select()
        .from(cupons)
        .orderBy(desc(cupons.data_criacao));
      return result;
    } catch (error) {
      logger.error('[DB] Erro ao buscar cupons:', { error });
      throw error;
    }
  }
  ...
  async getCupom(id: number): Promise<Cupom | undefined> {
    try {
      ...
      logger.error('[DB] Erro ao buscar cupom:', { id, error });
      throw error;
    }
  }

  async getCupomByCodigo(codigo: string): Promise<Cupom | undefined> {
    try {
      const [result] = await this.db
        .select()
        .from(cupons)
      ...
      logger.error('[DB] Erro ao buscar cupom por código:', { codigo, error });
      throw error;
    }
  }

  async createCupom(cupom: InsertCupom): Promise<Cupom> {
    try {
      ...
      logger.info('[CUPOM] Cupom criado', { codigo: created.codigo, tipo: created.tipo });
      return created;
    } catch (error: any) {
      if (error.message?.includes('duplicate key')) {
        throw new Error('Já existe um cupom com este código');
      }
      logger.error('[DB] Erro ao criar cupom:', { error });
      throw error;
    }
  }
  ...
  async updateCupom(id: number, updates: Partial<Cupom>): Promise<Cupom | undefined> {
    try {
      ...
      if (sanitizedUpdates.codigo) {
        sanitizedUpdates.codigo = sanitizedUpdates.codigo.toUpperCase();
      }
      ...
      logger.info('[CUPOM] Cupom atualizado', { id, updates: Object.keys(updates) });
      return updated;
    } catch (error) {
      logger.error('[DB] Erro ao atualizar cupom:', { id, error });
      throw error;
    }
  }
  ...
  async deleteCupom(id: number): Promise<boolean> {
    try {
      ...
      logger.info('[CUPOM] Cupom deletado', { id });
      return result.length > 0;
    } catch (error) {
      logger.error('[DB] Erro ao deletar cupom:', { id, error });
      throw error;
    }
  }
  ...
  async validarCupom(codigo: string, plano: string, userId: string): Promise<{ valido: boolean; cupom?: Cupom; erro?: string }> {
    try {
      const cupom = await this.getCupomByCodigo(codigo);
      ...
      if (cupom.status !== 'ativo') {
        return { valido: false, erro: 'Cupom inativo ou expirado' };
      }
      ...
      if (agora > dataExpiracao) {
        await this.updateCupom(cupom.id, { status: 'expirado' });
      ...
      if (cupom.planos_aplicaveis) {
        const planosAceitos = Array.isArray(cupom.planos_aplicaveis) 
          ? cupom.planos_aplicaveis 
          : [];
        
        if (planosAceitos.length > 0 && !planosAceitos.includes(plano)) {
          return { valido: false, erro: 'Cupom não aplicável a este plano' };
        }
      }

      // Verificar se o usuário já usou este cupom
      const usoAnterior = await this.db
        .select()
        .from(usoCupons)
        .where(
          and(
      ...
      logger.error('[CUPOM] Erro ao validar cupom:', { codigo, error });
      return { valido: false, erro: 'Erro ao validar cupom' };
    }
  }

  async usarCupom(cupomId: number, userId: string, subscriptionId: number, valorDesconto: number): Promise<UsoCupom> {
    try {
      ...
      logger.info('[CUPOM] Cupom utilizado', { cupomId, userId, valorDesconto });
      return uso;
    } catch (error) {
      logger.error('[DB] Erro ao registrar uso do cupom:', { cupomId, error });
      throw error;
    }
  }
  ...
  async getUsoCupons(cupomId?: number): Promise<UsoCupom[]> {
    try {
      let query = this.db.select().from(usoCupons);

      if (cupomId) {
        query = query.where(eq(usoCupons.cupom_id, cupomId)) as any;
      }

      const result = await query.orderBy(desc(usoCupons.data_uso));
      return result;
    } catch (error) {
      logger.error('[DB] Erro ao buscar uso de cupons:', { cupomId, error });
      throw error;
    }
  }
